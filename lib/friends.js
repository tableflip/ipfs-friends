...
class FriendDaemon extends EventEmitter {
  ...
  async start () {
    const index = await readFriends(this._ipfs)
    this._index = index
    const { id } = await this._ipfs.id()
    this.peerId = id
    const peerIds = Object.keys(index)
    await Promise.all(peerIds.map(peerId => {
      return this._ipfs.pubsub.subscribe(peerId, this.onMessage)
    }))
    this._ipfs.pubsub.publish(id, Buffer.from(JSON.stringify({ action: 'online' })))
  ...
  async onMessage (msg) {
    ...
    if (action === 'share') {
      ...
      console.log(this._index, peerId)
      this.emit('message:online', { peerId, name: this._index[peerId] })
    } else if (action === 'offline') {
      this.emit('message:offline', { peerId, name: this._index[peerId] })
    }
  }
